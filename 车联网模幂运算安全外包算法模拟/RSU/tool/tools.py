...
def mod_pow(a,b,p): #模幂运算
    return gmpy2.mpz(gmpy2.powmod(gmpy2.mpz(a),gmpy2.mpz(b),gmpy2.mpz(p)))


def inverse(a,p): #求逆元
    return gmpy2.mpz(gmpy2.invert(gmpy2.mpz(a),gmpy2.mpz(p)))


def mod_mul(a,b,p): #模乘法
    c =gmpy2.mpz(a*b)
    return gmpy2.mpz(c%p)
...
def mod_sub(a,b,p):
        return gmpy2.mpz(gmpy2.mod(a-b,p))
